      ...
      <q-page>
        <div class="q-ma-lg">
          <div class="row items-center">
            <q-btn unelevated round color=#E2E8F0 icon="img:icons/admin/AddDisaster/arrow-left.svg" to="/admin" />
            <p class="q-mb-none text-subtitle1 text-weight-bold">Pilih Lokasi Penampungan</p>
          </div>
          <q-input v-if="!addEvacuationLocationFlag" rounded outlined v-model="search" type="search" color="orange" placeholder="Cari Lokasi Penampungan"
            class="q-mt-sm q-mb-lg q-pt-sm" dense>
            <template v-slot:append>
              <q-icon name="search" />
            </template>
          </q-input>
          <div v-if="!addEvacuationLocationFlag" v-for="(evacLoc) in filteredEvacLocs" class="evacuation-card q-mb-md q-pa-md">
            <router-link :to="`/admin/evacuation/disaster-id/evacuation-id?disasterId=${evacLoc.disasterId}&locId=${evacLoc.id}&evacName=${evacLoc.evacName}&alamat=${evacLoc.alamat}&kecamatan=${evacLoc.kecamatan}&rt=${evacLoc.rt}&rw=${evacLoc.rw}&namaPetugas=${evacLoc.namaPetugas}&nomorPetugas=${evacLoc.nomorPetugas}`">
