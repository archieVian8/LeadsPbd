  ...
  methods: {
    async getApplicantsData() {
      try {
        ...
        if (response.data[0].Status === "Anda tidak memiliki izin untuk melihat pendaftar AcademicEvents") {
          Notify.create({
            color: 'red',
            message: 'Anda tidak memiliki izin untuk melihat pendaftar AcademicEvents',
            position: 'top',
            timeout: 2500
          });
